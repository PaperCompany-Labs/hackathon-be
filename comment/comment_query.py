...
def get_comments(db: Session, novel_shorts_no: int) -> CommentListResponse:
    try:
        comments = (
            db.query(Comment)
            .filter(Comment.novel_shorts_no == novel_shorts_no, not Comment.is_del)
...
def update_comment(db: Session, comment_no: int, user_no: int, update_data: CommentUpdate) -> CommentActionResponse:
    try:
        comment = (
            db.query(Comment).filter(Comment.no == comment_no, Comment.user_no == user_no, not Comment.is_del).first()
        )
...
def like_comment(db: Session, comment_no: int) -> CommentActionResponse:
    try:
        stmt = update(Comment).where(Comment.no == comment_no, not Comment.is_del).values(like=Comment.like + 1)
...
def dislike_comment(db: Session, comment_no: int) -> CommentActionResponse:
    try:
        stmt = (
            update(Comment)
            .where(Comment.no == comment_no, not Comment.is_del, Comment.like > 0)
